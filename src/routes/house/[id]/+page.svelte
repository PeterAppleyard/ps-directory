...
<main class="min-h-screen bg-white">
	...
	<div class="w-full bg-stone-900" style="aspect-ratio: 16/7; max-height: 60vh; overflow: hidden;">
		...
		{:else}
			...
			<div class="flex h-full w-full items-center justify-center px-8">
				<p class="font-black tracking-tight text-stone-700"
	...
	<section class="border-b-2 border-stone-900 bg-white">
		<div class="mx-auto max-w-6xl divide-y-2 md:divide-y-0 md:grid md:grid-cols-[320px_1fr] md:divide-x-2 divide-stone-900">
			...
			<div class="px-8 py-10 space-y-6">
				{#if house.style}
					<div>
						<p class="text-[10px] font-bold tracking-normal text-stone-400 mb-1">Style</p>
						<p class="font-black text-2xl tracking-tight text-stone-900">{house.style}</p>
					</div>
				{/if}

				<div>
					<p class="text-[10px] font-bold tracking-normal text-stone-400 mb-1">Location</p>
					<p class="font-black text-xl tracking-tight text-stone-900">{house.address_suburb}</p>
					<p class="text-sm text-stone-500 mt-0.5">{house.address_state} {house.address_postcode}</p>
				</div>

				{#if house.year_built}
					<div>
						<p class="text-[10px] font-bold tracking-normal text-stone-400 mb-1">Built</p>
						<p class="font-black text-xl tracking-tight text-stone-900">{house.year_built}</p>
					</div>
				{/if}

				{#if house.builder_name}
					<div>
						<p class="text-[10px] font-bold tracking-normal text-stone-400 mb-1">Builder</p>
				...
				{#if house.condition}
					<div>
						<p class="text-[10px] font-bold tracking-normal text-stone-400 mb-2">Condition</p>
						<span class="border px-3 py-1 text-[10px] font-bold tracking-normal {conditionColor(house.condition)}">
							{house.condition}
						</span>
					</div>
				{/if}
				...
				{#if house.listing_url || house.sold_listing_url}
					<div class="space-y-2 border-t border-stone-100 pt-4">
						<p class="text-[10px] font-bold tracking-normal text-stone-400 mb-3">Listings</p>
						{#if house.listing_url}
							<a
								href={house.listing_url}
								target="_blank"
								rel="noopener noreferrer"
								class="flex items-center justify-between border-2 border-stone-900 bg-stone-900 px-4 py-2.5 text-[10px] font-bold tracking-normal text-white transition hover:bg-white hover:text-stone-900"
							>
								<span>For Sale</span>
								<span>↗</span>
							</a>
						...
						{#if house.sold_listing_url}
							<a
								...
								target="_blank"
								rel="noopener noreferrer"
								class="flex items-center justify-between border-2 border-stone-200 px-4 py-2.5 text-[10px] font-bold tracking-normal text-stone-500 transition hover:border-stone-900 hover:text-stone-900"
							>
			...
			<div class="px-8 py-10">
				<h1 class="font-black text-4xl tracking-tight text-stone-900 mb-2 md:text-5xl">
					{house.address_suburb}
				</h1>
	...
	<section class="bg-stone-900 px-6 py-16">
		<div class="mx-auto max-w-6xl flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
			<p class="font-black text-2xl tracking-tight text-white md:text-3xl">
				Know another P&amp;S home?
			</p>
			<div class="flex flex-wrap gap-4">
				<a
					href="/submit"
					class="inline-block border-2 border-white bg-white px-6 py-3 text-xs font-bold tracking-normal text-stone-900 transition hover:bg-transparent hover:text-white"
				>
					Submit a Home
				</a>
				<a
					href="/"
					class="inline-block border-2 border-stone-600 px-6 py-3 text-xs font-bold tracking-normal text-stone-400 transition hover:border-white hover:text-white"
				>
					Back to Directory
				</a>
			</div>
...
{#if lightboxIndex !== null}
	...
	>
		<!-- Counter -->
		<p class="absolute top-4 left-6 text-[10px] font-bold tracking-normal text-stone-500">
		...
		<button
			...
			class="absolute top-4 right-6 text-[10px] font-bold tracking-normal text-stone-500 transition hover:text-white"
			aria-label="Close"
		>
			Close ✕
		</button>
		...
		{#if allImages.length > 1}
			<button
				...
				class="absolute left-4 top-1/2 -translate-y-1/2 border-2 border-stone-700 px-4 py-3 text-xs font-bold tracking-normal text-stone-400 transition hover:border-white hover:text-white"
			...
			<button
				...
				class="absolute right-4 top-1/2 -translate-y-1/2 border-2 border-stone-700 px-4 py-3 text-xs font-bold tracking-normal text-stone-400 transition hover:border-white hover:text-white"
